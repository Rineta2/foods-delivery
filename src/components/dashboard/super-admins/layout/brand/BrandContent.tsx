...
import { Brand } from './lib/interface'

import { brandService } from '@/components/dashboard/super-admins/layout/brand/lib/brandService'
...
export default function BrandContent() {
    ...
    const [newBrandStatus, setNewBrandStatus] = useState<boolean>(true);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editingBrand) {
                if (selectedFile) {
                    const imageUrl = await brandService.uploadImage(selectedFile);
                    await brandService.updateBrand(editingBrand.id, {
                        imageUrl: imageUrl as string,
                        title,
                        isActive: newBrandStatus
                    });
                } else {
                    await brandService.updateBrand(editingBrand.id, {
    ...
    const closeModal = () => {
        const modals = ['brand_modal', 'delete_modal', 'view_modal'];
            ...
            <div className="container">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select
                                            value={newBrandStatus.toString()}
                                            onChange={(e) => setNewBrandStatus(e.target.value === 'true')}
                ...
                <dialog id="delete_modal" className="modal">
                    <div className="modal-box custom-scrollbar bg-white">
                        ...
                        <p className="py-4">Are you sure you want to delete this brand?</p>
                        <div className="modal-action">
                            ...
                            <button
                                onClick={() => brandToDelete && handleDelete(brandToDelete.id)}
                        ...
                        {editingBrand && (
                            <div className="space-y-6">
                                ...
                                <div className="flex justify-between items-center">
                                    <h3 className="text-2xl font-bold text-gray-800">{editingBrand.title}</h3>
                                    <button
                                        onClick={closeModal}
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-4">
                                        <span className={`px-3 py-1 rounded-full text-sm ${editingBrand.isActive
                                            ? 'bg-green-100 text-green-800'
                                            : 'bg-red-100 text-red-800'
                                            }`}>
                                            {editingBrand.isActive ? 'Active' : 'Inactive'}
                                        </span>
                                        <span className="text-gray-500">
                                            Created: {new Date(editingBrand.createdAt).toLocaleDateString()}
                                        </span>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {currentItems.map((brand) => (
                        <div key={brand.id} className="group bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden border border-gray-100">
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex justify-between items-center mb-4">
                                    <select
                                        ...
                                        value={brand.isActive.toString()}
                                        onChange={() => handleToggleStatus(brand.id, brand.isActive)}
                                    >
                                        <option value="true">Active</option>
                                        <option value="false">Inactive</option>
                                    </select>
